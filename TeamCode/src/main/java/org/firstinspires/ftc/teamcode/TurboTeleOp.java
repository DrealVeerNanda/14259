...
public class TurboTeleOp extends LinearOpMode {
    ...
    // Control Parameters
    private final double armLowerBound = 0.5;
    private final double armUpperBound = 0.82;
    private final double clawLowerBound = 0.55;
    private final double clawUpperBound = 0.85;
    private final double frontArmLowerBound = 0.04;
    private final double frontArmUpperBound = 0.85;
    private final int linSlideLowerBound = 0;
    private final int linSlideUpperBound = 1150;
    private final double linSlidePower = 1;
    private final double depositLowerBound = 0.53;
    private final double depositUpperBound = 0.98;
    ...
    private final double[] depositPositions = { 0.0, 0.35, 1.0 };
    private int depositPosition = 0;
    private final double[] linearServoPositions = { 0.0, 0.3, 1.0 };
    private int linearServoPosition = 0;
    private final double[] frontArmDifference = {0 , 0.05, 0.14};
    private final double[] linSlideFactor = {1, 0.8, 0.5};
    ...
    private void initGamepads() {
        ...
        TurboTeleOp.bPressedReset = () -> {
            intakeOut();
            resetLinSlide();
            TurboTeleOp.bPressed = TurboTeleOp.bPressedTakeCone;
        };

        TurboTeleOp.bPressedTakeCone = () -> {
            intakeBack();
            TurboTeleOp.bPressed = TurboTeleOp.bPressedTakeCone;
        };

        TurboTeleOp.bPressedMoveCone = () -> {
            linSlideUp();
            TurboTeleOp.bPressed = TurboTeleOp.bPressedTakeCone;
        };

        TurboTeleOp.bPressedDumpCone = () -> {
            dump();
            TurboTeleOp.bPressed = TurboTeleOp.bPressedTakeCone;
        };

        this.gamepad1Wrapper.subscribeBPressedEvent(() -> {
            TurboTeleOp.bPressed.run();
            return true;
        });
    ...
    private void interact() {
        ...
        this.turret.getDcMotor().setPower(0.4 * (gamepad1.right_trigger - gamepad1.left_trigger));
...
//        telemetry.addData("R", this.odometryWrapper.getR() / this.degreesToTicks);
    ...
    private void moveDeposit(){
        ...
        this.deposit.setPosition(this.depositPositions[depositPosition] + 0.1 * linearServoPosition);
    }
