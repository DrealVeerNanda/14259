...
public class   LeftSide1Plus10 extends LinearOpMode {
    //Camera
    OpenCvCamera camera;
    ...
    private int turretUpperBound = 6600;
    ...
    private double[] linSlidePositions = { 0.0, 0.7, 0.7};
    ...
    private void initDrivetrain() {
        ...
        TwoWheelOdometryWrapper odometryWrapper = new TwoWheelOdometryWrapper()
                ...
                //.setPerpEncoder(hardwareMap.dcMotor.get("FrontEncoder"))
...
//    telemetry.addData("Angular V", imu.getAngularVelocity());
    ...
    public void runOpMode() throws InterruptedException {
        ...
        Trajectory traj5 = drive.trajectoryBuilder(traj6.end())
                .lineToLinearHeading(new Pose2d(53, -68, Math.toRadians(-90)))
                .build();
...
//            this.driveWrapper.setPose(0, 5, 0);
//            for(int i = 0; i < 7; i++) this.updateAll();
            drive.followTrajectory(traj4);
            this.linearServo.setPosition(0.32);
            this.turret.setPosition(0.10); //.17
            sleep(500);
            //this.driveWrapper.setPose(0, 50, 0);
            //for(int i = 0; i < 35; i++) this.updateAll();
            frontArm.setPosition(0.8);
            //this.driveWrapper.setPose(0, 50, -Math.PI / 2.0);
            //for(int i = 0; i < 13; i++) this.updateAll();
            //this.driveWrapper.setPose(10, 50, -Math.PI / 2.0);
            linSlidePosition = 2;
            moveLinSlide();
            //for(int i = 0; i < 4; i++) this.updateAll();
            depositPosition = 1;
            moveDeposit();
            linSlideUp(5);
            //for(int i = 0; i < 9; i++) this.updateAll();
            //parked = true;
            this.displayStats();
            armPosition = 5;
            linSlidePosition = 2;
            ...
            //this.driveWrapper.setPose(-52, 50, -Math.PI / 2.0);
            //for(int i = 0; i < 35; i++) this.updateAll();
            turret.setPosition(0.5);
            //this.driveWrapper.setPose(-52, 50, 0);
            //for(int i = 0; i < 20; i++) this.updateAll();
           // this.driveWrapper.setPose(-52, 50, Math.PI / 2.0);
            //for(int i = 0; i < 20; i++) this.updateAll();
            ...
            //parked = false;
            sleep(200);
            //this.driveWrapper.setPose(-68, 50, Math.PI / 1.8);
            for(int i = 0; i < 2; i++) this.updateAll();
            greatReset();
            for(int i = 0; i < 12; i++) this.updateAll();
            updateAll();
            sleep(500);
...
//            }
    ...
    private void intakeArm(int i){
        if(i == 5) frontArm.setPosition(frontArmPositions[frontArmPosition] + 0.16);
        else if(i == 4) frontArm.setPosition(frontArmPositions[frontArmPosition] + 0.12);
        else if(i == 3) frontArm.setPosition(frontArmPositions[frontArmPosition] + 0.08);
        else if(i==2) frontArm.setPosition(frontArmPositions[frontArmPosition] + 0.03 * i);
